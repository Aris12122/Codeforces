...
vector<int> get_ans(vector<option> &v) {
    ...
    // vector<vector<int>> pr(101, vector<int>(n+1, -1));
    dp[0][0] = 0;
    for (int k = 1; k <= n; k++) {
        auto [t,p,id] = v[k-1];
        dp[0][k] = 0;
    ...
    /*
    for (int i = 101; i < 200; i++) if (ckmin(t, dp[i])) {
        curp = i;
    }
    if (t == INF) return {-1};
    
    
    vector<bool> us(sz(v), false);

    for (int k = 0; k < sz(v); k++) if (!us[k]) {
        auto [t, p, id] = v[k];
        if (curp - p < 0) continue;
        if (dp[curp - p] == dp[curp] - t) {
            ans.emplace_back(id);
            curp -= p;
            us[k] = true;
            break;
